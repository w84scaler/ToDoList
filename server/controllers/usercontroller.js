...
exports.login = function(socket){
    socket.on("LOGIN", async function(data, callback) {
        ...
        if(user && user[0]){
            if(await bcrypt.compare(data.password, user[0].hash)){
                const _token = jwt.sign({login : data.login}, process.env.ACCESS_TOKEN_SECRET) ///, expires : date, {expiresIn : 60*1000} 1 arg is obj if use option /// can be a stirng if don't use /////////fuuuck
